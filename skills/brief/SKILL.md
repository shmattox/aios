...
2. **Decide whether to gather or reuse a recent cache.** Run the tested boolean — don't re-derive it:
   ...
   **Freshness is event-based, not a pure timer (A93 §1).** `--max-age-min` is now only a BACKSTOP
   ...
   - **Stale / missing / degraded (the NORMAL at-desk case):** run the full gather
     (`references/gather.md` → `# Gather`), populate the cache data (`## Cache contract`, in `references/contract.md`),
...
Moved verbatim to `references/contract.md` → `Cache contract — the two files (the tail of EVERY full gather)` — read that section when this step is reached.

## Surface (`profile: brief.surface`) — how it renders AND acts

Moved verbatim to `references/contract.md` → `Surface (`profile: brief.surface`) — how it renders AND acts` — read that section when this step is reached.
...
- **Decision:** `python "${CLAUDE_PLUGIN_ROOT}/engine/tools/brief_session.py" record_decision <state_path> <item_id> <station> <choice> "<action>" [--executed] [--thread T] [--title T]`
  ...
  - **Conclusion-write mandate (A95 §2 — a refused write with no recorded fallback is a BUG, not an outcome).** Every conclusion MUST land in a durable, gather-readable home at decision time — the walk must never conclude an item into thin air (else the next gather re-asks it cold):
    1. **flip the task field** when the target is flippable — `notion_writeback.py flip` (unchanged);
    2. **write a log row** to the owning silo's EXISTING session log (or decision log for a decision) — `python "${CLAUDE_PLUGIN_ROOT}/engine/tools/notion_writeback.py" add-row --db <log-db> --writable <log-db> --title "<conclusion>" --change-log <path>` — against the profile's `notion.write.writable.session_log`/`decision_log` groups (the per-teamspace Session/Decision Log DBs that already exist; NEVER create a new surface). The `pause_economic` content gate applies to row text exactly as to a flip;
    3. **AND** — when the natural Notion target is FENCED (a content field, an unlisted DB) — also write/update `<env_root>/state/threads/{id}.md` so the next gather (which reads threads first-class) still sees it. An install whose profile lacks a `session_log` group DEGRADES to threads-only (read the lever — never assume the reference profile's tables).
...
**What each scope renders** — the scope→stations table, the `--order` station tokens, the masthead echo, and
the two Dev-only panels (A73 gate metrics; Factory Standup + veto triage) are the render contract in
`references/contract.md` → `## Scope — what each scope renders`. Resume must match scope (the `# Walk ledger` guard).
...
Moved verbatim to `references/contract.md` → `Walk tracker (render at top of each station)` — read that section when this step is reached.
...
Moved verbatim to `references/contract.md` → `Graded voice — system (blue) and Claude (orange)` — read that section when this step is reached.

## Brainstorm-packet decision cards (A77 — after the walk render, before done)

Moved verbatim to `references/contract.md` → `Brainstorm-packet decision cards (A77 — after the walk render, before done)` — read that section when this step is reached.

## Sync proposals (A96 — the "Sync proposes" approval panel, inside the front door)

Moved verbatim to `references/contract.md` → `Sync proposals (A96 — the "Sync proposes" approval panel, inside the front door)` — read that section when this step is reached.
...
Moved verbatim to `references/contract.md` → `Windows are durable threads (`state/threads/`)` — read that section when this step is reached.

# Render — per item + layout

Moved verbatim to `references/contract.md` → `Render — per item + layout` — read that section when this step is reached.
...
Moved verbatim to `references/contract.md` → `Render — per held item` — read that section when this step is reached.

## Batch panel — when the lane is big (A15)

Moved verbatim to `references/contract.md` → `Batch panel — when the lane is big (A15)` — read that section when this step is reached.

## Review surface — conversational by default, widget on request (`brief.surface`, as `## Surface`)

Moved verbatim to `references/contract.md` → `Review surface — conversational by default, widget on request (`brief.surface`, as `## Surface`)` — read that section when this step is reached.
